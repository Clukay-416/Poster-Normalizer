...
async function request(path,body,method){const options=body===undefined?{}:{method:method||'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)};const response=await fetch('/api'+path,options);if(response.status===401){location.href='/';throw Error('请先登录')}if(!response.ok){let err;try{err=await response.json()}catch{err={detail:response.statusText}}throw Error(typeof err.detail==='string'?err.detail:JSON.stringify(err.detail))}return response.json()}
